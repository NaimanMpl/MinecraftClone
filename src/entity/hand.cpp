...
Hand::Hand() {
    ...
    viewBobbing = new ViewBobbing{0.0f, .1f, 0.1f, 0.3f, glm::vec3(0.0f), 0.0f};
}


void Hand::update(glm::vec3 cameraRight, glm::vec3 velocity) {

    if (animation == HandAnimation::Moving) {
        float magnitude = sqrt(pow(velocity.x, 2.0f) + pow(velocity.y, 2.0f) + pow(velocity.z, 2.0f));

        if (magnitude > 0.0f) {
            viewBobbing->sinTime += viewBobbing->effectSpeed;
        } else {
            viewBobbing->sinTime = 0.0f;
        }

        viewBobbing->y = -abs(viewBobbing->effecIntensity * sin(viewBobbing->sinTime)) * 0.1f;
        viewBobbing->position = cameraRight * viewBobbing->effecIntensity * float(cos(viewBobbing->sinTime)) * viewBobbing->effectIntensityX * 0.1f;
    } else {
